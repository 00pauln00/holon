...
class Recipe(HolonRecipeBase):
    ...
    def run(self, clusterobj):
        print(f"=================Run Term Ticker recipe ====================\n")
        ...
        print("After restart, term value is: %s" % curr_term)

        if curr_term <= before_restart_term:
            print("curr_term %s <= before_restart_term %s" % (curr_term, before_restart_term))
            recipe_failed = 1

        if recipe_failed:
            print("Term Ticker Recipe Failed")
        else:
            print(f"Term ticker Recipe Successful, Raft Peer 0's term value increasing!!\n")
