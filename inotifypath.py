       ...
       # print("miliseconds", miliseconds)
        ...
        if input_dir:
            ...
            if input_base == inotify_input_base.SHARED_INIT:
                ...
                fpath = "%s/%s.%s" % (self.inotify_shared_init_path, timestamp, base_fname)
            elif input_base == inotify_input_base.PRIVATE_INIT:
                fpath = "%s/%s/init/%s.%s" % (self.inotify_path, peer_uuid, timestamp, base_fname)
            else: # input_base = REGULAR
                fpath = "%s/%s/%s/%s.%s" % (self.inotify_path, peer_uuid, dir_name, timestamp,
                                          base_fname)
