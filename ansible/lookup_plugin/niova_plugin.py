...
def niova_raft_process_ops(raft_conf, peer_idx, operation):
    ...
    print("raft_uuid: %s" % raft_uuid)
    print("peer_uuid: %s" % peer_uuid)
    print("idx: %s" % peer_idx)
    print("ops: %s" % operation)
    process_type ="server"
...
class LookupModule(LookupBase):
    def run(self, terms, **kwargs):
        ...
        raft_json_fpath = "%s/%s.json" % (recipe_params['base_dir'], recipe_params['raft_uuid'])
        ...
        if operation == "ctlrequest":
            ...
            raft_conf = niova_ctlreq_cmd_create(raft_conf, peer_idx, ctlreq_cmd_dict)
        elif operation == "raftconfigure":
            config_params_dict = terms[2]
            raft_conf = niova_raft_conf_create(recipe_params, config_params_dict)
        elif operation == "raftprocess":
            print("raftprocess operation")
            peer_idx = terms[2]
            proc_operation = terms[3]
            raft_conf = niova_raft_process_ops(raft_conf, peer_idx, proc_operation)
        elif operation == "recipe_verify":
            compare_src = terms[2]
            rule_table = terms[3]
            result = niova_raft_recipe_verify(raft_conf, compare_src, rule_table)
        elif operation == "lookup_raft_key":
            peer_idx = terms[2]
            ctlreq_cmd_dict = terms[3]
            result = niova_raft_lookup_key(raft_conf, peer_idx, ctlreq_cmd_dict)

        if operation != "recipe_verify" and operation != "lookup_raft_key":
